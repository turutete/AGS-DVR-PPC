...
function check_enum()
   -- XXX no es necesaria comprobaci�n, no permitimos edici�n, solo seleccionar de la lista
   return true
end
...
function check_date(s, args)
   ...
   for k,v in pairs(args.fields) do
      ...
      -- Comprobamos que est�n todos los campos
      if not d[k] then
	 return false
      end
   end
   ...
   if d.month==2 then
      local dias_feb=28 -- por defecto, 28 d�as
	 ...
	 dias_feb=29
      ...
      -- comprobamos d�a v�lido
      if d.day>dias_feb then
	 return false
      end
   end

   -- Comprobamos meses de 30 d�as
