...
-- Devuelve los elementos que est�n en "a" pero no en "b"
...
local function insert_log_row(sds,descr,time,element,cond, init)
   ...
   if not init then
	 ...
	 if not flag_stop and lines>=LOG_MAX then -- XXX
      ...
      -- Emitir notificaci�n de nueva fila insertada en hist�rico
...
function alarmlogtable_new(params)
   ...
   local del_log = function()
			 ...
			 nextkey=accessx.getnextkey(sds, nextkey) -- siguiente
		      ...
		      -- inicializamos
		      id=1
		      -- XXX a�adir un evento "borrado de hist�rico"
   ...
   return {
      set=set,
      del_log=del_log,
      -- exportamos funci�n para insertar una fila sin control de estado
      insert=insert,
