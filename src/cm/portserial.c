...
xMBPortSerialInit( UCHAR ucPort, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity )
{
    ...
    //printf("modbus>>> INICIALIZACION DEL PUERTO\n");
...
xMBPortSerialPoll(  )
{
	...
	if( prvbMBPortSerialRead( &ucBuffer[0], BUF_SIZE, &usBytesRead ) )
        {
            if( usBytesRead == 0 )
            {
                //printf("modbus>>> Timeout de LECTURA en SerialPoll\n"); fflush(stdout);
		...
		for( i = 0; i < usBytesRead; i++ )
                {
                    //printf("modbus>>> LECTURA _byte_ en SerialPoll (%d): %02X\n", i+1, ucBuffer[i]); fflush(stdout);
    ...
    if( bTxEnabled )
    {
        while( bTxEnabled )
        {
	    //printf("modbus>>> TX en SerialPoll (%d)\n", uiTxBufferPos); fflush(stdout);
            ( void )pxMBFrameCBTransmitterEmpty(  );
            /* Call the modbus stack to let him fill the buffer. */
        }
        //printf("modbus>>> WRITE en SerialPoll\n"); fflush(stdout);
	...
	/* Eliminar ECO */
	   ...
	   /* Idea vaciar buffer de recepcion tras envio para evitar el eco de nuestra implementacion 485!
	   tcflush( iSerialFd, TCIFLUSH );
	   tcflush( iSerialFd, TCIFLUSH );
	   printf("jur> Varios TCIFLUSH\n"); fflush(stdout);  // KO!!!
	   */
	   
	   numBytesEnviados = uiTxBufferPos;
	   do {
	      if (prvbMBPortSerialRead( &ucBuffer[0], BUF_SIZE, &usBytesRead ) == TRUE) {
	         numBytesEnviados -= usBytesRead;
	      }
	      else {  //seguridad
	         numBytesEnviados = 0;
	      }
	      //printf("modbus>>> LECTURA de ECO. Bytes leidos: %d. Por leer: %d\n", usBytesRead, numBytesEnviados); fflush(stdout);
	   } while( numBytesEnviados > 0 );
	}
	#endif
    }
